...
validates it into L1-ready `xr.Dataset`s for the UK, India, and NL regions.
...
asset (`l0_ecmwf_live_local`) before being parsed. UK and India share a schema
(`EcmwfLiveUkIndiaSchema`); NL has its own (`EcmwfLiveNlSchema`), reflecting a different forecast
horizon and variable set for that region.

## Layout

- `schema.py` — `EcmwfLiveUkIndiaSchema` and `EcmwfLiveNlSchema`, the pandera schemas these
...
- `processing.py` — `_read_raw_grib` / `_transform` (region-agnostic parsing), and
  `process_ecmwf_live_uk_india` / `process_ecmwf_live_nl` (per-region fetch/transform/validate
  entrypoints), split as described in the root README.
